...
class Editor:
    ...
    def delet_ann(self, annotation_id=None):
        """
        删除标注
        - 提供 annotation_id → 删除单条标注
        - 不提供 → 删除整张图片所有标注
        """
        if annotation_id is not None:
            self.dataset_explorer.delete_annotation(self.image_id, annotation_id)
            print(f"[INFO] 删除 image_id={self.image_id}, annotation_id={annotation_id}")
        else:
            self.dataset_explorer.clear_annotations_for_image(self.image_id)
            print(f"[INFO] 删除 image_id={self.image_id} 下所有标注")
    ...
    def next_category(self):
        self.category_id = (self.category_id + 1) % len(self.categories)

    def prev_category(self):
        self.category_id = (self.category_id - 1 + len(self.categories)) % len(self.categories)
