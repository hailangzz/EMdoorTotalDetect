...
class DisplayUtils:
    ...
    def __convert_ann_to_mask(self, ann, height, width):
        """
        将 COCO annotation 转为二值 mask
        兼容 polygon 和 RLE（解决贴边 mask 崩溃问题）
        """
        mask = np.zeros((height, width), dtype=np.uint8)
        seg = ann["segmentation"]

        # ---------------------------
        # 1️⃣ 判断 segmentation 类型
        # ---------------------------
        if isinstance(seg, dict) and "counts" in seg:  # RLE
            ...
            try:
                ...
                print(f"[WARN] Failed to convert polygon to RLE, fallback: {e}")
                return mask  # polygon 转 RLE 失败 → 返回空 mask
        ...
        # mask_instance 可能是 HxWx1 → 压缩成 HxW
        if mask_instance.ndim == 3:
            mask_instance = mask_instance[:, :, 0]
